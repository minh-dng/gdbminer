...
class CoverageFuzzer:
    ...
    def __init__(self, grammar: Grammar):
        ...
        self.uncovered = {
            symbol: set(range(len(expansions))) for symbol, expansions in grammar.items()
        }
    ...
    def _choose_rule(self, symbol: str, depth: int, max_depth: int) -> list[str]:
        ...
        if depth >= max_depth:
            ...
            candidates = [index for index, cost in enumerate(self.cost[symbol]) if cost == minimum]
        ...
        while cursor < len(chart):
            ...
            if symbol in grammar:
                ...
                for completed in list(chart):
                    ...
                    if (
                        comp_lhs == symbol
                        and comp_dot == len(comp_rule)
                        and comp_origin == position
                    ):
