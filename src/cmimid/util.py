...
def collect_deletable_nodes(tree):
    name, children, *rest = tree
    matching_nodes = []

    for child in children:
        matching_nodes.extend(collect_deletable_nodes(child))

    if name.endswith(Epsilon + ">") and not contains_empty_node(tree):
        matching_nodes.append(tree)

    return matching_nodes


def contains_empty_node(tree):
    name, children, *rest = tree
    ...
    **args,
    ...
    if log:
        with open("do.log", "a+") as f:
            print(
                json.dumps({"cmd": command, "env": env, "exitcode": result.returncode}),
                env,
                flush=True,
                file=f,
            )
...
def is_a_replaceable_with_b(a1, a2, module):
    n1, f1, t1 = a1
    n2, f2, t2 = a2
    if tree_to_str(n1) == tree_to_str(n2):
        return True
